...
_mode3:
    ...
    LDI		BIT_ITER, 7
    LDI		RX_BYTE, 0					// clear the rx buffer byte
    LDI		TRANS_COUNTER, 0				// no bytes transferred yet
    LBBO	TX_BYTE, MEM_START, TRANS_COUNTER, 1		// load the first byte
    CLR		CS_PINS, ID_REG					// drop CS
// LOOP
    QBEQ	_mode3End, TRANS_COUNTER, TRANS_TOTAL
//----------------------------------------------------CLOCK LOW ---
    CLR		CLOCK_PIN					// drop clock
    LSR		TEMP_BYTE, TX_BYTE, BIT_ITER			// prepare data (transition)
// wait
// clock up
// read input
// wait
